...
import { analyzeWebsite, fetchHistoryById } from "@/lib/api";
...
function ScanPageContent() {
  ...
  const normalizeStoredScan = (scan: any): ScanResult => ({
    report: scan?.report,
    raw: scan?.raw || {
      uiPatterns: {
        hasNavbar: false,
  ...
  useEffect(() => {
    ...
    if (scanIdParam) {
      ...
      void (async () => {
        try {
          const storedScan = await fetchHistoryById(scanIdParam, user.uid);
          setScannedUrl(storedScan?.url || "");
          setResult(normalizeStoredScan(storedScan));
          setActiveTab("report");
        } catch (err: any) {
  ...
  }, [searchParams, handleScan, user?.uid]);
...
export default function ScanPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-[#080c10]" />}>
      <ScanPageContent />
    </Suspense>
  );
}